...
in
{
  ...
  programs = {
    ...
    zsh = {
      ...
      shellAliases = {
        rebuild = "git -C ~/jaysaflake add -A && nix fmt ~/jaysaflake && sudo nixos-rebuild switch --flake ~/jaysaflake"; # Can later explore colmena and other build + deploy options... this works fine for now i think
      };
    };
  };
}
